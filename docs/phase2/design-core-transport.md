...
**前置依赖：** Phase 1 P0 实验已完成（见 [p0-experiment-walkthrough.md](../phase1-results/p0-experiment-walkthrough.md)）
...
| # | 结论 | 证据章节 | Phase 2 含义 |
|---|------|---------|-------------|
| ① | 单个 Write WR 内部丢一包 → 整个 WR 作废，概率 `1-(1-p)^c`（c = WR 的 MTU 包数） | [walkthrough §7.3](../phase1-results/p0-experiment-walkthrough.md#73-验证-1full-严格符合-1-p256) | `ChunkManager` 必须把大 buffer 切成**多个独立 WR**，每个 WR 的 `c` 足够小使得 `(1-p)^c` 接近 1 |
| ② | 接收端 CQE（`IBV_WC_RECV_RDMA_WITH_IMM`）是全送达的**充要条件** | [walkthrough §7.4](../phase1-results/p0-experiment-walkthrough.md#74-验证-2cqe--full-永远相等) | `RatioController` 只数 CQE，**绝不扫 buffer**——扫 buffer 毫无意义（无法区分"新数据恰好等于 OLD_PATTERN"和"旧数据"） |
| ③ | 污染模式是**纯前缀截断**（3000 轮 0 CORRUPT） | [walkthrough §7.5](../phase1-results/p0-experiment-walkthrough.md#75-验证-3-0-corrupt--纯前缀截断) | `GhostMask` 每个 chunk 只需 `{bool has_cqe; size_t valid_len;}`，**不需要 bitmap**（比 per-word bitmap 省 1000×） |
| ④ | 几何丢包模型成立（`avg_first_old_off` 匹配截断几何分布 ±3%） | [walkthrough §7.6](../phase1-results/p0-experiment-walkthrough.md#76-验证-4avg_first_old_off-符合截断几何分布) | Phase 2 的仿真实验可以**继续沿用软件丢包注入**作为真机 `netem` 的替代，审稿可辩护 |

另外 [walkthrough §7.9](../phase1-results/p0-experiment-walkthrough.md#79-为什么这条陡峭的-1-p256-曲线并不判-uc-死刑) 澄清的 "UC 两层语义"（WR 内部硬语义 vs. WR 之间互不干扰）是整个 Phase 2 架构的**前提假设**——没有这条假设，切 chunk 就没有意义。
