...
- Mellanox ConnectX-5 / ConnectX-6 for real evaluation (Phase 3 Stage B, CloudLab)
  ...
  - **Prior polishing platform (2026-04-23 → 2026-04-27)**: `amd203.utah.cloudlab.us` + `amd196.utah.cloudlab.us` with ConnectX-5 (fw 16.28.4512, 25 GbE, RoCEv2 GID idx 1). Multi-ACTIVE-port nodes (mlx5_0 on 128.110.x management + mlx5_2 on 10.10.1.x experiment LAN); `scripts/cloudlab/detect_rdma_dev.sh` + `day0_check.sh` prefer `enp<bus>s<slot>f<func>np<port>` naming to pick the experiment port. CPU-only torch (no GPU). Stage B matrices archived under `docs/phase3/results-cx5-amd203-amd196/`.
  - **Current polishing platform (2026-04-28+)**: `amd247.utah.cloudlab.us` (rank 0 / receiver) + `amd245.utah.cloudlab.us` (rank 1 / sender) + `amd264.utah.cloudlab.us` (XDP middlebox), all amd-class Utah CloudLab. ConnectX-5 (fw 16.28.4512, **25 GbE on `eno34np1`**, mlx5_1 = experiment LAN, RoCEv2 GID idx 1). Each node also has a ConnectX-5 Ex / MT28800 with QSFP28 100 GbE ports (`enp65s0f0np0`, mlx5_2/mlx5_3) but the profile does not wire those — `ethtool` reports `No partner detected` (verified 2026-04-28). Auto-detect now uses RFC1918 (10.x/192.168.x) IPv4 to pick the experiment iface, so both `eno34np1` and `enp65s0f0np0` work without manual override. Same 25 GbE NIC class + firmware as amd203/amd196 → all post-fix tunings (chunk_bytes=4096, sq_depth=8, timeout_ms=200) carry over without re-sweep. Confirmed baselines: ib_write_bw 24.39 Gbps, ib_write_lat 8 B p99 2.19 µs. Stage B matrices on this cluster will land under `docs/phase3/results-cx5-amd247-amd245/`.
  - see `docs/phase3/history/stage-b-hardware-notes.md` §8 (c240g5 archive) / §9 (amd203/amd196 CX-5) / §10 (amd247/amd245 CX-5)
  - see `docs/phase3/cluster-amd247-amd245-amd264.md` for the current cluster's IP/MAC/dev fact sheet
